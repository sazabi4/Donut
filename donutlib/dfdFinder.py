...
sys.path.insert(1, "/software/products/Donut-trunk/python/Donut/donutcode/")
...
def findDonuts(dataAmp, xOffset, inputDict):
    ...
    nDonutWanted = inputDict["nDonutWanted"]  # no divide 2 for DESI
    ellipCut = inputDict["ellipCut"]

    # info about CCDs for desici
    # dinfo = desiciinfo()

    # info about CCDs for desi gfa
    dinfo = desiinfo()
    ...
    skyBkg = calcSky(dataBlock)  #####pring skyBkg
    ...
    for i in range(numOverThreshold):
        ...
        if dataOn[iyx] == 1:
            ...
            if nOk > npixelsMinCut:
                ...
                # calculate rdecam, only needed for DECam, not DESI
                ...
                if nOk < npixelsMaxCut and flux > fluxMinCut and flux < fluxMaxCut and numpy.abs(
                        ellip1) < ellipCut and numpy.abs(ellip2) < ellipCut:
...
def dfdFinder(hdulist_extName, inputDict={}):
    ...
    defaultDict = {"nBlock": 16,
                   ...
                   "kNNFind": 40,
                   "distanceLimit": 2.8,
                   "fluxMinCut": 2.0e6,  # 2e5 is the original number
                   "fluxMaxCut": 3.0e7,
                   "npixelsMinCut": 5,
                   "npixelsMaxCut": 25,
                   "nDonutWanted": 12,
    ...
    data = hdulist_extName.data
    # header = hdulist_extName.header # header is never used anywhere
    ...
    # This is for CI instrument
    # dataAmpA = data[0:2047,0:3071]

    #for DESI (after using David Kirkby's detrending code)
    dataAmpA = data
    # This is for GFA, and remove 50 pixel overscan on both side and 100 pixel in the middel
    # dataAmpA = data[:, numpy.r_[50:1074, 1174:1175]]

    # print dataAmpA.shape
    # dataAmpA = data[516:, 50:1074]
    # dataAmpB = data[516:, 1174:2198]

    # find em
    donutListA = findDonuts(dataAmpA, 0, defaultDict)
    # donutListB = findDonuts(dataAmpB,0,defaultDict)
